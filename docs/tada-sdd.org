...
- Dataq SVC config:
  + stored in typical/expected config place
  + allow set of host/port for BOTH data-queue and redis
  + allow default values for all queues (set once instead of N times)
  + maximum_errors_per_"RECORD" => "FILE"

** Metadata
As expected, the fields in metadata of fits are ad-hoc.  Even the file
naming standards makes assumptions about what is available and I've
found that what it assumes isn't always true.   This will continue to
be a fact of life for our chain.  I think the "solution" is to make it
easy and clear how to programmatically add metadata when it can be
done automatically.  It may also be useful to provide a stand-alone
tool that can be used to add fields with user interaction.
