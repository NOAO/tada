...
* TODO Missing in Thread-4
- [ ] install all python in github and PyPI in pythonic way
  + use wheel
  + universal format
...
- [ ] svc (dqpush, dqpop) need to be made into a single actual "dataq" service
  + that can started/stopped/restarted in the normal linux way.
...
- [ ] change irods: demoResc -> tadaResc
- [ ] change irods: /tempZone -> tadaZone
- [ ] (somehow) avoid having to do "iinit"
...
- [ ] clean out debugging
- [ ] replace stubs (tada/submit.py)
- [ ] different dir (irods) struct for ingest?

* TODO Next thread
I think the next thread won't just be a thread.  It will be a working
prototype. The first MVP.

- actually ingest into archive (alternate archive? dry-run option?)
- refactor puppet
- add puppet modules for my stuff (dq,tada,astro)
...
* TODO missing
There are TONS of things still missing in this thread.  I stopped
adding to my list after a while because the list was sooo long.  Here
are just some highlights to serve as reminders.  There are pieces of
lists all over the place (whiteboard, paper,
files). Regroup. Consolodate.
...
- svc (dqpush, dqpop) need to be made into a single actual "dataq" service
  + that can started/stopped/restarted in the normal linux way.
- scrape all code for "!!!" indicator of stuff that needs fixing.
- document underlying data structure 
  + This includes all the /var/tada/* subdirs, the irods
    "/tempZone/valley" structure, and all 3 queues
- change irods: demoResc -> tadaResc
- change irods: /tempZone -> tadaZone
- (somehow) avoid having to do "iinit"
- special user or group for this to all run under?
- rename puppet-dci2
- make sure all logs go to /var/log/tada/
- make sure all configs come from /etc/tada  ???
- make sure all services write pid to /var/run/tada
- Write management script clear/reset everything:
  + rm -rf /var/tada/*/*
  + dqcli --clear
- Write monitor scripts to scrape from logs and queues
  + ERRORS and WARNINGS
