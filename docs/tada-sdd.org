...
*** Simulation
Would be GREAT generally connect simulator to data-flow graph
...
** Requirements
I don't really know what the requirements are!  My guess is they
generally include:
1. Provide all the functionality of the existing system
2. Resilient 
   - don't break -- EVER
3. Maintainable
   - by new employees without large learning curve
4. Be fast enough (need quantification)

** More requirements
*** <2014-10-10 Fri>
JH: all software must be capable of reconnecting to database servers on connection loss
ie, so components can be restarted

** Requirements derived from "tubes_notes.txt"
1. Increase level of automation of regular operation functions
2. use version control always; with commit comments
3. elliminate direct changes to live production system (from tagged version)
4. (document minimumaly acceptable coding style)
5. Implement regression testing (automated where possible, documented otherwise)
6. write design documentation
7. write installation documentation
8. write usage documentation
9. reproducible installs
10. daily operations must not require manual intervention
11. daily operations must not require human monitoring (automatic alerts instead)
12. eliminate metadata remmediation in its present form (what form???)
    + get metadata from file format, or
    + get metadata from TO/observer/observatory support staff at data
      collection time
13. insert "archival metadata" just before final archiving (???)
14. insert of archival metadata should be idempotent
15. eliminate mountain copy coherency requirement (???)
16. filename agnostic; nothing in the system should depend on the
    structure or uniqueness of a filename

** requirement for DQ
DciTrackD limits access to specific clients (hosts via specified
ports)

#+BEGIN_SRC perl
my %clients = ();    # client access (s)ervices;
if($opt_c) {
    open CLIENTS,"<$opt_c" or die "Cannot find clients file $opt_c.  Exiting";
    while (<CLIENTS>) {
        my ($mach,$services) = split(/ /,$_);
        chomp $services;
        foreach my $kv (split ',',$services) {
            my ($p,$s) = split "=",$kv;
            if($p == $port) {
                # clients 'access' allowed on this 'port'.
                $clients{$mach} = $s;
                last;
            }
        }
...
#+TITLE:   TADA :: Telescope Automatic Data Archiver
